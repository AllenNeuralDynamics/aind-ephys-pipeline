...
// hash:sha256:f8327fa6314d4ed38ea96ba87ce693a4611a37d915eb75a02e9f49af318fe20a
...
process capsule_aind_ephys_spikesort_kilosort_25_7 {
	tag 'capsule-7634370'
	container "$REGISTRY_HOST/published/fca2ac1b-4c23-468b-aa49-eb7040270796:v3"
	...
	export CO_CAPSULE_ID=fca2ac1b-4c23-468b-aa49-eb7040270796
	export CO_CPUS=16
	...
	echo "[${task.tag}] cloning git repo..."
	if [[ "\$(printf '%s\n' "2.20.0" "\$(git version | awk '{print \$3}')" | sort -V | head -n1)" = "2.20.0" ]]; then
		git -c credential.helper= clone --filter=tree:0 --branch v3.0 "https://\$GIT_ACCESS_TOKEN@\$GIT_HOST/capsule-7634370.git" capsule-repo
	else
		git -c credential.helper= clone --branch v3.0 "https://\$GIT_ACCESS_TOKEN@\$GIT_HOST/capsule-7634370.git" capsule-repo
	fi
	mv capsule-repo/code capsule/code && ln -s \$PWD/capsule/code /code
	rm -rf capsule-repo
